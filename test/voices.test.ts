...
describe('音色:预置 + 克隆授权', () => {
  ...
  it('复刻未产出 voice_id → status=failed(不可用,避免误生成)', () => {
    ...
    expect(voices.isUsableVoice(v.id, T, U)).toBe(false);
  });
  it('isUsableVoice:预置 + 本租户 ready(复刻成功)可用,跨租户不可用', () => {
    ...
    expect(voices.isUsableVoice('Cherry', T, U)).toBe(true);
    expect(voices.isUsableVoice(v.id, T, U)).toBe(true);
    expect(voices.isUsableVoice(v.id, 'other', U)).toBe(false);
  });
});
