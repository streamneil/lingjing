...
describe('C6 重命名 + 设为默认', () => {
  it('重命名', () => {
    ...
    expect(renameAvatar(av.id, T, '新名', U)).toBe(true);
    expect(listCustom(T, U).find((x) => x.id === av.id)!.name).toBe('新名');
    expect(renameAvatar(av.id, 'other', 'x', U)).toBe(false); // 跨租户不可改
  });
  it('设为默认互斥(同一用户只有一个默认)', () => {
    ...
    expect(setDefaultAvatar(a.id, T, U)).toBe(true);
    expect(getDefaultAvatar(T, U)!.id).toBe(a.id);
    setDefaultAvatar(b.id, T, U); // 切换默认
    expect(getDefaultAvatar(T, U)!.id).toBe(b.id); // 只有 b 是默认
    expect(listCustom(T, U).filter((x) => x.is_default === 1).length).toBe(1);
  });
});
...
describe('自定义形象 + 授权存证', () => {
  ...
  it('列表 + 删除 + 租户隔离', () => {
    ...
    expect(listCustom(T, U).length).toBe(1); // 只看自己租户
    const mine = listCustom(T, U)[0]!;
    expect(deleteAvatar(mine.id, T, U)).toBe(true);
    expect(listCustom(T, U).length).toBe(0);
  });
  ...
  it('isUsableAvatar:预置可用、本租户 ready 可用、跨租户不可用', () => {
    ...
    expect(isUsableAvatar('preset-1', T, U)).toBe(true);
    expect(isUsableAvatar(av.id, T, U)).toBe(true);
    expect(isUsableAvatar(av.id, 'other', U)).toBe(false); // 别的租户用不了
    expect(isUsableAvatar('ghost', T, U)).toBe(false);
  });
});
