...
全新部署后开箱即数据完整,且**不依赖任何外部公共桶**(去中心化):
- **默认积分套餐**:app 启动时自动种子(表空才灌、幂等);裸 `docker compose up` 也生效。
- **示范素材**(落地页/探索灵感/果茶示范/预置形象/音色试听):随 git 入库 → 进镜像 →
  `deploy.sh` 自动 seed 到你**自己的 OSS**(`showcase/` 前缀);前端走 `/api/showcase-asset`
  (签名重定向自己桶,桶里没有则伺服镜像内文件兜底)。

DB 里只差「厂商 key + 租户账号」,运营走:
...
> 数据完整性:示范素材 + 预置形象声音随仓自带,默认积分套餐启动自种子。运营**只需配厂商 key +
> 建租户**即可开张。默认套餐可在 `/admin` 改价/增删,幂等不覆盖你改过的。
