...
// ── Qwen-TTS HTTP 合成(声音设计音色用)──
// 查证(2026-06,阿里官方):Qwen-TTS 走 HTTP MultiModalConversation
// (POST /services/aigc/multimodal-generation/generation),与 CosyVoice WebSocket 协议不同。
// 响应可能是 output.audio.url(签名 URL,24h 过期)或 output.audio.data(base64 内联),
// 随模型/参数而异 → 双解析,统一返 Buffer(与 synthesizeSpeech 同口径,worker 统一 putObject)。

/**
 * Qwen-TTS HTTP 合成单段文本,返音频 Buffer。
 * voice 为设计/复刻产出的 Qwen voice_id;model 为 qwen3-tts-vd-* 等。
