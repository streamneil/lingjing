...
import { isUsableAvatar, listPresets as listAvatarPresets, getAvatarWithinTenant } from '../avatars/index.js';
...
async function signAvatarThumb(avatarRef: string | undefined, tenantId: string): Promise<string | null> {
  ...
  // 调用方的自有 job 已过 scopeByOwner 过滤,归属在上游确认 → 按租户解析缩略图
...
// 账号隔离:video/tts 的形象/音色校验需 actingUserId;其余 builder 不用,忽略多余参数。
