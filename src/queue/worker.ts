...
import { listPresets as listAvatarPresets, getAvatarForWorker } from '../avatars/index.js';
import { isPreset as isPresetVoice, getVoiceForWorker } from '../voices/index.js';
...
async function resolveImageUrl(avatarRef: string, tenantId: string): Promise<string> {
  ...
  // worker 无 acting user(异步处理);提交时已做账号校验(isUsableAvatar),此处走 worker 专用租户级解析。
  const custom = getAvatarForWorker(avatarRef, tenantId);
...
export function resolveVoice(
  ...
  // worker 无 acting user;提交时已账号校验,此处走 worker 专用租户级解析。
  const v = getVoiceForWorker(voiceRef, tenantId);
